...
class RedirectDeliveryAddressTest extends TestCase
{
    ...
    public function testRedirectedDeliveryAddress()
    {
        var_dump("*****1st_Method:testRedirectedDeliveryAddress");
        $users =
            [
                [
                    'name' => 'cat',
        ...
        //変更処理
        $response = $this->post(route('purchase.redirect'), [
            'item_id' => $item->id,
            'post_code' => '123-1234',
            'address' => 'newAddress',
        ...
        $response->assertSee('newAddress');      // ステータスが表示されているか
        $response->assertSee('newBuilding');      // ステータスが表示されているか
        ...
        $response->assertStatus(200); //正常終了コード
        //dump($response);
        $stripeToken = 'tok_visa';
        $delivery_address = $profile->post_code . $profile->address . $profile->building;
        $payment_method = "コンビニ払い";
        ...
        $response->assertStatus(302); //正常コード/リダイレクト
